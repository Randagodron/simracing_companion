...
from source.richard_burns_rally import udp_data
from source.richard_burns_rally import car_data_rbr
from source.richard_burns_rally import track_data_rbr


class GameRichardBurnsRally(GameBase):

    valid_game_name_rbr = 'Richard_Burns_Rally'
    current_save_version = '1.0.0'

    def __init__(self, game_name):
        self.unknown_cars = set()
        self.unknown_tracks = set()
        if game_name != GameRichardBurnsRally.valid_game_name_rbr:
            raise ValueError('Invalid game name: {}'.format(game_name))
        self.game_name = game_name

    @staticmethod
    def get_valid_game_names():
        return [GameRichardBurnsRally.valid_game_name_rbr]
    ...
    def save_data(self, data, file_path):
        np.savez_compressed(file_path, samples=data, game=self.game_name,
                            save_version=GameRichardBurnsRally.current_save_version)
